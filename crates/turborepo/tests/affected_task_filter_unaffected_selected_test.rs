...
fn filtering_to_unaffected_package_does_not_keep_dependency_only_tasks() {
    ...
    // Only alpha#test is directly affected. Its required beta#build dependency
    // enters the affected engine as execution support. Filtering to beta must
    // not reinterpret that dependency as an affected beta entrypoint.
    fs::write(
